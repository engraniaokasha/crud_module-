    ...
    public function extendWith(Menu $menu)
    {
        $menu->group(trans('core::sidebar.content'), function (Group $group) {
            $group->item(trans('crud::cruds.title.cruds'), function (Item $item) {
                ...
                $item->item(trans('crud::users.title.users'), function (Item $item) {
                    $item->icon('fa fa-copy');
                    $item->weight(0);
                    $item->append('admin.crud.user.create');
                    $item->route('admin.crud.user.index');
                    $item->authorize(
                        $this->auth->hasAccess('crud.users.index')
                    );
                });
// append
